...
/* ------------------------------------------------------------------
   ...
   - Edit your own array, then watch adjacent pairs get compared and
     swapped so the largest value "bubbles" to the right each pass.
   - Bars: height = value. Indigo = unsorted, sky = the pair being
     compared, emerald = the sorted tail that is locked in.
...
function buildFrames(a0: number[]): Frame[] {
  ...
  const snap = (compare: number[], sortedFrom: number, caption: string) =>
    frames.push({ arr: [...a], compare, pivot: [], sorted: range(sortedFrom, n), caption });

  frames.push({ arr: [...a], compare: [], pivot: [], sorted: [], caption: 'Each pass walks left to right, bubbling the largest value to the end.' });
  let sortedFrom = n;
  for (let i = 0; i < n - 1; i++) {
    let swapped = false;
    for (let j = 0; j < n - i - 1; j++) {
      snap([j, j + 1], sortedFrom, `Compare ${a[j]} and ${a[j + 1]}.`);
      if (a[j] > a[j + 1]) {
        [a[j], a[j + 1]] = [a[j + 1], a[j]];
        swapped = true;
        snap([j, j + 1], sortedFrom, `${a[j + 1]} > ${a[j]} → swap them.`);
      } else {
        snap([j, j + 1], sortedFrom, `${a[j]} ≤ ${a[j + 1]} → already in order, leave them.`);
      }
    }
    sortedFrom = n - i - 1;
    snap([], sortedFrom, swapped ? `End of pass ${i + 1}: ${a[sortedFrom]} is parked in its final spot.` : 'A full pass with no swaps — the array is already sorted!');
    if (!swapped) break;
  }
  frames.push({ arr: [...a], compare: [], pivot: [], sorted: range(0, n), caption: 'Sorted! Every neighbour is now in order.' });
  return frames;
}
...
export default function SortBubble() {
  ...
  return (
    <div class="not-prose rounded-2xl border border-border bg-surface p-4 shadow-sm">
      ...
      <p class="mt-2 text-center text-xs text-muted">Sky = the pair being compared · Amber = active · Emerald = locked-in sorted tail.</p>
    </div>
  );
}
