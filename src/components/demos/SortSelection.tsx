...
/* ------------------------------------------------------------------
   ...
   - Each pass scans the unsorted part for the minimum (amber = current
     smallest so far, sky = element being compared), then swaps it to
     the front of the unsorted region. Emerald = the sorted prefix.
   ------------------------------------------------------------------ */
...
function buildFrames(a0: number[]): Frame[] {
  ...
  frames.push({ arr: [...a], compare: [], pivot: [], sorted: [], caption: 'Find the smallest value, swap it to the front, then repeat on the rest.' });
  for (let i = 0; i < n - 1; i++) {
    let min = i;
    snap([], [min], i, `New pass: assume the smallest is ${a[i]} at position ${i}.`);
    for (let j = i + 1; j < n; j++) {
      snap([j], [min], i, `Compare ${a[j]} with current smallest ${a[min]}.`);
      if (a[j] < a[min]) {
        min = j;
        snap([], [min], i, `${a[min]} is smaller — it is the new minimum.`);
      }
    }
    if (min !== i) {
      [a[i], a[min]] = [a[min], a[i]];
      snap([], [i], i, `Swap the minimum ${a[i]} into position ${i}.`);
    } else {
      snap([], [i], i, `${a[i]} was already the smallest — no swap needed.`);
    }
    snap([], [], i + 1, `Position ${i} is locked in.`);
  }
  frames.push({ arr: [...a], compare: [], pivot: [], sorted: range(0, n), caption: 'Sorted! Notice it always scanned the whole rest — selection sort never speeds up.' });
  return frames;
}
...
export default function SortSelection() {
  ...
  return (
    <div class="not-prose rounded-2xl border border-border bg-surface p-4 shadow-sm">
      ...
      <p class="mt-2 text-center text-xs text-muted">Amber = smallest found so far · Sky = element being compared · Emerald = sorted prefix.</p>
    </div>
  );
}
