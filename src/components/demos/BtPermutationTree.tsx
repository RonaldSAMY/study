...
/* ------------------------------------------------------------------
   Animated backtracking tree for PERMUTATIONS (used[] template).
   - Edit a short list. The demo fixes one unused element per level,
     recording every choose / explore / un-choose step.
   - The root -> active path is the arrangement built so far (indigo);
     a complete ordering is accepted in emerald; a duplicate value that
     would repeat an arrangement is pruned in red (try [1, 1, 2]).
...
const parseList = (s: string): number[] =>
  s.split(',').map((x) => parseInt(x.trim(), 10)).filter((x) => Number.isFinite(x)).slice(0, 4);
...
function buildTrace(input: number[]): Trace {
  ...
  const rec = (path: number[], nodeId: number, depth: number) => {
    frames.push({ id: nodeId, action: 'visit', caption: path.length ? `arrangement so far: [${path.join(', ')}]` : 'start with an empty arrangement' });
    if (path.length === n) { solutions++; frames.push({ id: nodeId, action: 'accept', caption: `complete permutation [${path.join(', ')}] ✓` }); return; }
    for (let i = 0; i < n; i++) {
      if (used[i]) continue;
      if (i > 0 && arr[i] === arr[i - 1] && !used[i - 1]) {
        const cid = newNode(nodeId, depth + 1, String(arr[i]));
        frames.push({ id: cid, action: 'visit', caption: `try ${arr[i]} at position ${path.length + 1}` });
        frames.push({ id: cid, action: 'prune', caption: `${arr[i]} repeats an earlier unused ${arr[i - 1]} → prune to skip duplicate orderings` });
        continue;
      }
      const cid = newNode(nodeId, depth + 1, String(arr[i]));
      used[i] = true; path.push(arr[i]);
      rec(path, cid, depth + 1);
      path.pop(); used[i] = false;
      frames.push({ id: nodeId, action: 'backtrack', caption: `un-place ${arr[i]} — back to [${path.join(', ')}]` });
    }
  };
  rec([], root, 0);
...
export default function BtPermutationTree() {
  const [text, setText] = useState('1, 2, 3');
  const [input, setInput] = useState<number[]>(() => parseList('1, 2, 3'));
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const cssW = Math.max(parent.clientWidth, cols * 46 + 60);
  ...
  const commit = () => { const p = parseList(text); if (p.length) { setInput(p); setIdx(0); setPlaying(false); } };
  ...
  return (
    <div class="not-prose rounded-2xl border border-border bg-surface p-4 shadow-sm">
      <div class="mb-3 flex flex-wrap items-center gap-2">
        <input value={text} onInput={(e) => setText((e.target as HTMLInputElement).value)} class="w-44 rounded-lg border border-border bg-surface-2 px-3 py-1.5 font-mono text-sm" placeholder="up to 4 numbers" />
        <button onClick={commit} class="rounded-lg bg-brand px-3 py-1.5 text-sm font-semibold text-white hover:opacity-90">Load</button>
        <span class="ml-auto text-xs text-muted">permutations: <strong class="text-text">{trace.solutions}</strong></span>
      </div>
      ...
      <div class="mt-3 flex flex-wrap gap-3 text-xs text-muted">
        <span><span class="mr-1 inline-block h-3 w-3 rounded-full align-middle" style="background:#4f46e5" />arrangement so far</span>
        <span><span class="mr-1 inline-block h-3 w-3 rounded-full align-middle" style="background:#10b981" />complete permutation</span>
        <span><span class="mr-1 inline-block h-3 w-3 rounded-full align-middle" style="background:#ef4444" />duplicate (pruned)</span>
      </div>
      <p class="mt-2 text-center text-xs text-muted">Tip: try <code>1, 1, 2</code> to watch the red duplicate-pruning that turns 6 leaves into 3.</p>
    </div>
  );
}
